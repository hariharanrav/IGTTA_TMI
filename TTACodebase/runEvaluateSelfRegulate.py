...
if __name__ == '__main__':
    ...
    for ind,data in tqdm.tqdm(enumerate(testloader)):             
        ...
        if 'Const' in mode:
            maxConf = 1000
            lambdas = np.round(np.linspace(0.1,1.0,10),3)
            winLamb = 100
            winflag =  np.array([True, 1000])
            for lamb in lambdas:
                print(" Doing batch number ", ind, " lamb ", lamb, " winLamb ", winLamb)
                tent_model = LoadadaptModel(net, stepsIter, predBase,gpuid, lamb)
                predTentLamb, flag = getpredLabelsAdapt(tent_model,inpTensor)
                predTentLamb = predTentLamb.cpu().detach()
                # del tent_model
                print(flag)
                print(flag[2][0], " flag Conf")
                if flag[0]:
                    continue
                if flag[2][0] < maxConf:
                    maxConf = flag[2][0]
                    winLamb = lamb
                    predTent = predTentLamb
                    winflag = flag
            predBase = predBase.cpu().detach()
            print( " \n \n Winning Lambda is ", winLamb, " For batch ", ind, " with conf ", maxConf, " flag ", winflag)
            print(" ############### ")
        else:
        ...
        if compute:
            ...
            tempArray = np.array([ind,diceABase, diceATent, diceNBase, diceNTent, str(winflag) + " " + str(winLamb),np.average(temp==1), np.average(temp==2), np.average(temp==0)])
            print(tempArray)
