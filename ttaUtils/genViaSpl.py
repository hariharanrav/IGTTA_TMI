...
class gen_via_spl(nn.Module):
    ...
    def forward(self, x):
        ...
        for count in range(self.steps):
            ...
            if 'AEsurr' in self.losstype:
                outputs, lossTrack = forward_and_adaptAE(x, self.model, self.optimizer, self.gpu, self.surrModel)
            elif 'surr' in self.losstype:
        ...
        if 'surr' in self.losstype:
            ...
            if (lossTrack >=lossInit) or (count < 1):
...
# forward_and_adapt(x, self.reference, self.lossFn,self.model, self.optimizer, self.gpu)
def forward_and_adaptSurrogate(x, model, optimizer, gpu, surrModel):
    print("Making grad false for surr model")
    surrModel.requires_grad_(False)
    output = model(x)
    soft = F.softmax(output,dim=1)
    inpSurr =  torch.cat([x, soft], dim=1)
    errMap = surrModel(inpSurr)
    ...
    for name, param in model.named_parameters():
        ...
        #break
    print(" After backward surrModel \n \n \n ")
    for name, param in surrModel.named_parameters():
        print(name, param.grad)
        #break
    optimizer.zero_grad()
...
def forward_and_adaptAE(x, model, optimizer, gpu, surrModel):
    # print("Making grad false for AE model")
    # surrModel.requires_grad_(False)
    
    output = model(x)
    soft =  F.softmax(output,dim=1)
    # _, predLabels = torch.max(soft,dim=1)
    # print(Counter(torch.flatten(predLabels.cpu().detach()).numpy()))
    #print(output.size(), soft.size(), softAE.size())   
    predAE = surrModel(soft)
    
    softAE = F.softmax(predAE, dim=1)
    _, softAELabels= torch.max(softAE,dim=1)
    # print(output.size(), soft.size(), softAE.size())
    
    mse = nn.MSELoss()  
    totLoss = mse(soft, softAE)
    # print(" After backward \n \n \n")
    # for name, param in model.named_parameters():
    #     print(name, param.grad)
    #     break
    ...
    # CEloss = nn.CrossEntropyLoss()
    # totLoss = CEloss(output, softAELabels)
    print(totLoss)
    # totLoss.requires_grad = True
    totLoss.backward()
    optimizer.step()
    ...
    optimizer.zero_grad()
    soft =soft.detach()
    softAE = softAE.detach()
    predAE = predAE.detach()
    # predLabels = predLabels.detach()
    return output.cpu().detach(), totLoss.cpu().detach().numpy()

def forward_and_adapt(x, reference,  lossFn, model, optimizer, gpu, lamb, bs = 16):
    """
    ...
    #print(outputs.size(), reference.size())
...
def configure_modelIL(model, layerID=2):
    ...
    print(" Inside Configure model input layer")
    
    for m in model.modules():
        if isinstance(m, nn.Conv2d):
            print(m, "configure model")
