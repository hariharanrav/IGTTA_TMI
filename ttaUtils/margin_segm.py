...
class margin(nn.Module):
    """margin adapts a model by entropy minimization during testing.

    Once margined, a model adapts itself by updating on every forward.
    """
    def __init__(self, model, optimizer, steps=1, episodic=False, gpu=1):
        ...
        self.steps = steps
        assert steps > 0, "margin requires >= 1 step(s) to forward and update"
...
def configure_model(model):
    """Configure model for use with margin."""
    # train mode, because margin optimizes the model to minimize entropy
    model.train()
    # disable grad, to (re-)enable only what margin updates
    model.requires_grad_(False)
    # configure norm for margin updates: enable grad + force batch statisics
...
def check_model(model):
    """Check model for compatability with margin."""
    is_training = model.training
    assert is_training, "margin needs train mode: call model.train()"
    param_grads = [p.requires_grad for p in model.parameters()]
    has_any_params = any(param_grads)
    has_all_params = all(param_grads)
    assert has_any_params, "margin needs params to update: " \
                           "check which require grad"
    assert not has_all_params, "margin should not update all params: " \
                               "check which require grad"
    has_bn = any([isinstance(m, nn.BatchNorm2d) for m in model.modules()])
    assert has_bn, "margin needs normalization for its optimization"
