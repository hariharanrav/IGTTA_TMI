...
import numpy as np
class llr(nn.Module):
    """llr adapts a model by entropy minimization during testing.

    Once llred, a model adapts itself by updating on every forward.
    """
    def __init__(self, model, optimizer, reference, gpu, steps=1, lamb=1,episodic=False):
        ...
        assert steps > 0, "llr requires >= 1 step(s) to forward and update"
    ...
    def forward(self, x):
        ...
        for count in range(self.steps):
            ...
            if count == 0:
                # lossInit  = llrloss(self.logits,self.gpu).cpu().detach().numpy()
                lossInit = lossTrack
                lossPrev = lossTrack
                outputsPrev = outputs
                if self.steps == 1:
...
#     print(mse(predictions_1.to(torch.float), reference.to(torch.float)))
    # adapt
    lossllr = llr_loss(outputs,gpu)
    lossConst = boundloss(outputs, reference, gpu)
    # print(lossConst, lossGCE)
    # totLoss =  lossGCE + 10000 * lossConst
    totLoss =  1 * lossllr + lamb*lossConst
    print("LLR : ", lossllr, " Bound: ", lossConst, "Lamb: ", lamb) #, "Total: ", totLoss)
    ...
    optimizer.step()
    optimizer.zero_grad()
    return outputs, [lossllr.cpu().detach().numpy(), lossConst.cpu().detach().numpy()]
