...
import norm
import numpy as np
class MarginMax_segm(nn.Module):
...
def forward_and_adapt(x, reference,  model, optimizer, gpu, lamb):
...
#     print(mse(predictions_1.to(torch.float), reference.to(torch.float)))
    # adapt
    lossMargin = marginLoss(outputs,gpu)
    lossConst = boundloss(outputs, reference, gpu)
    # print(lossConst, lossGCE)
    # totLoss =  lossGCE + 10000 * lossConst
    totLoss =  1 * lossMargin + lamb*lossConst
    print("Margin : ", lossMargin, " Bound: ", lossConst, "Lamb: ", lamb) #, "Total: ", totLoss)
    ...
    optimizer.step()
    optimizer.zero_grad()
    return outputs, [lossMargin.cpu().detach().numpy(), lossConst.cpu().detach().numpy()]
